...
module.exports = function analyzeHoldem(hand) {
  ...
  // track how many players saw each street
  ...
  analyzed.playersTo = { preflop: playerCount }
  const advanceStreet = () => {
    const current = analyzed.playersTo[nextStreet] + 0
    nextStreet = nextStreetMap[nextStreet]
    analyzed.playersTo[nextStreet] = current
  }
  const reducePlayerCount = () => {
    analyzed.playersTo[nextStreet]--
  }
  ...
  for (i = 0; i < hand.showdown.length; i++) {
    ...
    if ((p.type === 'show' || p.type === 'muck') &&
      player.cards == null && p.card1 != null && p.card2 != null && p.card3 != null && p.card4 != null) {
      player.cards = { card1: p.card1, card2: p.card2, card3: p.card3, card4: p.card4 }
    } else if (p.type === 'collect') {
